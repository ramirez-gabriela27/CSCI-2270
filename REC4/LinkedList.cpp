...
bool LinkedList::deleteAtIndex(int n)
{
  ...
  // Special case to delete the head
    ...
    Node *pres = head; // start with a present pointing to the first
	Node *prev = NULL; // assume previous is null

    for (int i = 0; i < n; i++){  // loop through until you find the index
        prev = pres;
        pres = pres -> next;
    }
    prev->next = pres->next;  // point to next to deleted
    delete pres;              // delete the present one
    isDeleted = true;
